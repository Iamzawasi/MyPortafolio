...
matrixArray = [];
function create_Matrix(size = 4) {
  //when size=6, then table=6x6...
  matrixArray = [];
...
function sum_up_neighbours(state, row, col) {
  ...
  if (matrixArray[row + 1] != undefined) {
    matrixArray[row + 1][col] != undefined && matrixArray[row + 1][col]
      ? (count += 1)
      : (count = count);
    matrixArray[row + 1][col + 1] != undefined && matrixArray[row + 1][col + 1]
      ? (count += 1)
      : (count = count);
    matrixArray[row + 1][col - 1] != undefined && matrixArray[row + 1][col - 1]
      ? (count += 1)
  ...
  matrixArray[row][col - 1] != undefined && matrixArray[row][col - 1]
    ? (count += 1)
    : (count = count);
  matrixArray[row][col + 1] != undefined && matrixArray[row][col + 1]
    ? (count += 1)
